module DeployerConfig
  ...
  # note: deployer repo is not open source yet, you'll have to deploy via `docker stack deploy manually from VM1 (ip: 172.17.0.4)`
  ...
  def deployer_config_push
    exe "cd #{PATH}/vendor/deployer-config && git add config.yml && git commit -m 'dk-infra - add new stack' && git push origin master"
  end
